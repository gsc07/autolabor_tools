...
#include "autolabor_pro2_driver/Encode.h"
#include "dynamic_reconfigure/server.h"
#include "autolabor_pro2_driver/SimAutolaborDriverConfig.h"
...
typedef autolabor_pro2_driver::SimAutolaborDriverConfig SimAutolaborDriverConfig;
typedef autolabor_pro2_driver::Encode Encode;
...
private:
  ...
  double radio_;
  int send_left_, send_right_;
  int receive_fl_, receive_fr_, receive_bl_, receive_br_;
  int delta_fl_, delta_fr_, delta_bl_, delta_br_;
  int current_fl_, current_fr_, current_bl_, current_br_;
...
void SimpleDriver::run(){
  ...
  if (init()){
    ...
    send_timer_ = node.createTimer(ros::Duration(1.0/control_rate_), &SimpleDriver::send_callback, this);
...
void SimpleDriver::distribute_msg(uint8_t msg_type, uint8_t* buffer_data){
  ...
  case SPEED_MSG_ID:{
    ...
    std::cout << "MSG_ID : " << (int)buffer_data[3] << std::endl;
    receive_fl_ = buffer_data[5] * 256 + buffer_data[6];
    receive_fr_ = buffer_data[7] * 256 + buffer_data[8];
    receive_bl_ =  buffer_data[9] * 256 + buffer_data[10];
    receive_br_ =  buffer_data[11] * 256 + buffer_data[12];

    cal_pulse(current_fl_, receive_fl_, delta_fl_);
    cal_pulse(current_fr_, receive_fr_, delta_fr_);
    cal_pulse(current_bl_, receive_bl_, delta_bl_);
    cal_pulse(current_br_, receive_br_, delta_br_);

    Encode receive;
    receive.fl = (int)(delta_fl_);
    receive.fr = (int)(delta_fr_);
    receive.bl = (int)(delta_bl_);
    receive.br = (int)(delta_br_);
...
void SimpleDriver::send_callback(const ros::TimerEvent&){
  if (run_flag_){
    ...
    data[9] = (send_left_ >> 8) & 0xff;
    data[10] = send_left_ & 0xff;
    data[11] = (send_right_ >> 8) & 0xff;
    data[12] = send_right_ & 0xff;
    check(data, 13, data[13]);
    boost::asio::write(*port_.get(), boost::asio::buffer(data, 14), ec_);
    Encode encode;

    encode.fl = send_left_;
    encode.bl = send_left_;
    encode.fr = send_right_;
    encode.br = send_right_;
