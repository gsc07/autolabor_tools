...
ChassisDriver::ChassisDriver():msg_seq_(0), start_flag_(true),current_fl_(0),current_bl_(0),current_fr_(0),current_br_(0){}
...
void ChassisDriver::handle_speed_msg(uint8_t* buffer_data){  
  ...
  ROS_DEBUG_STREAM("recive -> left: " << delta_fl_ << "|" << delta_bl_ << "(" << receive_fl_ << "|" << receive_bl_ << ")"
                   << "; right: " << delta_fr_ << "|" << delta_br_ << "(" << receive_fr_ << "|" << receive_br_ << ")");
  ...
  if (delta_time_ >= (0.5 / control_rate_)){
    int delta_left_ = -(delta_fl_ + delta_bl_)/2;
    int delta_right_ = -(delta_fr_ + delta_br_)/2;
